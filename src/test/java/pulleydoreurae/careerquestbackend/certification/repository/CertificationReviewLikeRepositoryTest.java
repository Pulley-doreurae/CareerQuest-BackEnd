 ...
 */
@DataJpaTest
@DisplayName("자격증 후기 좋아요 구현체 DB 테스트")
class CertificationReviewLikeRepositoryTest {
	...
	void findListByPostTest() {
		...
		PostLike postLike2 = CertificationReviewLike.builder().userAccount(user2).post(post).build();
		PostLike postLike3 = CertificationReviewLike.builder().userAccount(user3).post(post).build();
		PostLike postLike4 = CertificationReviewLike.builder().userAccount(user4).post(post).build();
		PostLike postLike5 = CertificationReviewLike.builder().userAccount(user5).post(post).build();
	...
	@Test
	@DisplayName("한 사용자가 작성한 댓글들을 불러오는 테스트")
